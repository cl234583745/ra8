...
calc_func(ee_s16 *pdata, core_results *res)
{
    ...
    {                             /* otherwise calculate and cache the result */
        ...
        switch (flag)
        {
            case 0:
                if (dtype < 0x22) /* set min period for bit corruption */
                    dtype = 0x22;
                retval = (ee_s16)core_bench_state(res->size,
                                          res->memblock[3],
                                          res->seed1,
                                          res->seed2,
                                          dtype,
                ...
                break;
            case 1:
                retval = (ee_s16)core_bench_matrix(&(res->mat), dtype, res->crc);
                if (res->crcmatrix == 0)
        ...
        retval &= 0x007f;
        *pdata = (ee_s16)((data & 0xff00) | 0x0080 | retval); /* cache the result */
        return retval;
    }
}
...
core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
    ...
    for (i = 0; i < size; i++)
    {
        ee_u16 datpat = ((ee_u16)((ee_u32)seed ^ i) & 0xf);
        ee_u16 dat
            = (ee_u16)((datpat << 3) | ((ee_u16)(i & 0x7))); /* alternate between algorithms */
        info.data16 = (ee_s16)((dat << 8) | dat);  /* fill the data with actual data and
    ...
    while (finder->next != NULL)
    {
        if (i < size / 5) /* first 20% of the list in order */
            finder->info->idx = (ee_s16)i++;
        else
        {
            ee_u16 pat = (ee_u16)((ee_u32)(i++) ^ (ee_u32)seed); /* get a pseudo random number */
