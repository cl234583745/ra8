...
//#define CONFIG_USB_HS_FIFO_32BIT
/***********************************************************************************************************************
 * Macro definitions
 **********************************************************************************************************************/
...
#ifdef CONFIG_USB_HS_FIFO_32BIT
#ifdef USB_HIGH_SPEED_MODULE
        if (USB_IS_USBHS(p_ctrl->p_cfg->module_number))
        {
            cfg |= R_USB_PIPECFG_CNTMD_Msk;
        }
#endif
#endif
    }
...
static inline bool pipe_xfer_in (usbd_instance_ctrl_t * const p_ctrl, uint8_t num)
{
    ...
    if (hs_module)
    {
        *d0fifosel = num | R_USB_FIFOSEL_MBW_32BIT;
        ...
        if (len)
        {
            uint32_t              * src32  = (uint32_t *) buf;
            volatile uint32_t     * fifo32 = (volatile uint32_t *) d0fifo;
            uint16_t                i;
            const uint16_t          words32 = len >> 2;
            const uint16_t          tail    = len & 3;
            ...
            if (tail)
            {
                *d0fifosel = num | R_USB_FIFOSEL_MBW_8BIT;
                ...
                volatile uint8_t * fifo8 = (volatile uint8_t *) ((uintptr_t) d0fifo + 3);
                uint8_t          * src8  = (uint8_t *) src32 + (words32 << 2);
        ...
        if (len > 0)
        {
            ...
            if (tail)
            {
                *d0fifosel = num | R_USB_FIFOSEL_MBW_8BIT;
                ...
                volatile uint8_t * fifo8 = (volatile uint8_t *) ((uintptr_t) d0fifo + 3);
                uint8_t          * dst8  = (uint8_t *) dst32 + (words32 << 2);
