...
{
    UINT32 i;
    UINT8 *u8_ptmp = (UINT8 *)pData;
    UINT16 u16cnt;

    for (i = 0; i < 2; i++)
    {
        if (sTxPDOassign.aEntries[i] == 0x1A00) // copy TxPDO-Map(0x1A00) entries
        {
            for(u16cnt =0 ; u16cnt < TxPDOMap.u16SubIndex0; u16cnt++)
            {
                if (((TxPDOMap.aEntries[u16cnt] & 0xFFFF0000)>>16) == 0x6000)
                {
                    UINT32 *pInputs = (UINT32 *)u8_ptmp;
                    *pInputs = InputCounter;
                    u8_ptmp += 4;
                }
            }
        }
        else if (sTxPDOassign.aEntries[i] == 0x1BFF) // copy Device User TxPDO-Map(0x1BFF) entries
        {
...
void APPL_OutputMapping(UINT16* pData)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (sRxPDOassign.aEntries[i] == 0x1601) // copy RxPDO-Map(0x1601) entries
        {
            for(u16cnt =0 ; u16cnt < RxPDOMap.u16SubIndex0; u16cnt++)
            {
                if (((RxPDOMap.aEntries[u16cnt] & 0xFFFF0000)>>16) == 0x7010)
                {
                    UINT32 *pOutputs = (UINT32 *)u8_ptmp;
                    OutputCounter = *pOutputs;
                    u8_ptmp += 4;
                }
            }
        }
        else if (sRxPDOassign.aEntries[i] == 0x17FF) // copy Device User RxPDO-Map(0x17FF) entries
        {
