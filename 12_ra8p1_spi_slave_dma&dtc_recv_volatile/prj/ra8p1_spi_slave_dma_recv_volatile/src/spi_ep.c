...
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_Read(&g_spi_slave_ctrl, g_slave_rx_buff, RX_MAX, SPI_BIT_WIDTH_32_BITS);
#else
    err = R_SPI_Read(&g_spi_slave_ctrl, g_slave_rx_buff, RX_MAX, SPI_BIT_WIDTH_32_BITS);
#endif
    /* Error handle */
    if(err != FSP_SUCCESS)
    {
